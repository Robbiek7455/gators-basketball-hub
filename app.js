...
(function () {
  ...
  function diag(msg) {
    ...
    d.innerHTML += `<div class="card"><pre class="tiny">${esc(msg)}</pre></div>`;
  }
  ...
  /* ---------- Banner / Hero (pointer-events fixed) ---------- */
  var HERO_IMAGES = [
  ...
  function renderHeroSlides() {
    var mount = $("#heroImages");
    if (!mount) return;
    mount.innerHTML = HERO_IMAGES.map(
      (src, i) => `<div class="slide${i === 0 ? " active" : ""}" style="background-image:url('${src}')"></div>`
    ).join("");
  }
  function setHero(i) {
    var mount = $("#heroImages");
  ...
  function startHero() {
    ...
    $("#heroPrev")?.addEventListener("click", () => {
      heroIdx = (heroIdx - 1 + HERO_IMAGES.length) % HERO_IMAGES.length;
      setHero(heroIdx);
    });
  ...
  function fillSeasonSelects() {
    var opts = SEASONS.map(
      (y) => `<option value="${y}" ${y === STATE.season ? "selected" : ""}>${y - 1}-${String(y).slice(-2)}</option>`
    ).join("");
  ...
  function espnTeam() {
    ...
    return fetch(url).then(ok).then(j);
  }
  ...
  function espnSummary(eventId) {
    var url = `${ESPN_BASE}/summary?event=${eventId}`;
    return fetch(url).then(ok).then(j);
  }

  /* ==================== LOADERS ==================== */

  // Roster + Stats (players & team) from ESPN team endpoint
  async function loadRosterAndStats() {
    try {
      ...
      STATE.roster = athletes.map((a) => ({
        ...
        headshot:
          a.headshot?.href ||
          `https://source.boringavatars.com/beam/96/${encodeURIComponent(a.displayName)}`,
        hometown: a.homeTown || ""
      }));

      // team + player per-game statistics
      const cats = data?.team?.statistics?.splits?.categories || data?.team?.team?.statistics?.splits?.categories || [];
      const teamStats = cats.reduce((acc, c) => {
        (c.stats || []).forEach((s) => (acc[s.name] = +s.value || 0));
        return acc;
      }, {});
      const players = (data?.team?.athletes || []).flatMap((g) =>
        (g.items || []).map((a) => {
          const pcats = a?.statistics?.splits?.categories || [];
          const m = pcats.reduce((acc, c) => {
            (c.stats || []).forEach((s) => (acc[s.name] = +s.value || 0));
            return acc;
          }, {});
          return {
      ...
      $("#playerStats").innerHTML =
        '<div class="card"><p class="tiny muted">Stats unavailable.</p></div>';
    }
  }

  // Schedule from ESPN (regular + postseason)
    ...
    try {
      ...
      const games = items.map((ev, idx) => {
        ...
        return {
          ...
          score:
            uf.score && opp.score ? `${uf.score}-${opp.score}` : "",
          box:
            ev.links?.find((l) => /boxscore/i.test(l.text || ""))?.href ||
      ...
      $("#scheduleWrap").innerHTML =
        '<div class="card"><p class="tiny muted">Schedule unavailable.</p></div>';
    }
  }

  // Analytics from ESPN game summary
  function poss(t) {
    return (t.fieldGoalsAttempted || 0) + 0.475 * (t.freeThrowsAttempted || 0) - (t.offensiveRebounds || 0) + (t.turnovers || 0);
  }
  function efg(t) {
    const fgm = t.fieldGoalsMade || 0,
      fg3 = t.threePointFieldGoalsMade || 0,
      fga = t.fieldGoalsAttempted || 0;
    return fga ? (fgm + 0.5 * fg3) / fga : 0;
  }
  function tor(t) {
    const to = t.turnovers || 0,
      fga = t.fieldGoalsAttempted || 0,
      fta = t.freeThrowsAttempted || 0;
    const denom = fga + 0.475 * fta;
    return denom ? to / denom : 0;
  }
  ...
  function renderSchedule() {
    ...
    $$("#scheduleTable .game-row").forEach((tr) =>
      tr.addEventListener("click", () => {
        ...
        var det = $(`#scheduleTable [data-det="${id}"]`);
        if (det) det.style.display = det.style.display === "none" ? "" : "none";
      })
    );
    $$("#scheduleTable .addCal").forEach((b) =>
  ...
  $("#exportScheduleCsv")?.addEventListener("click", () => {
    ...
    STATE.schedule.forEach((g) =>
      rows.push([g.date, g.time, g.opponent, g.at, g.result || "", g.score || "", g.tv || "", g.venue || "", g.city || "", g.box || ""])
    );
    const a = document.createElement("a");
    a.href = URL.createObjectURL(new Blob([toCSV(rows)], { type: "text/csv" }));
    a.download = "gators_schedule.csv";
    a.click();
  });
  function downloadICS(g) {
    ...
    const ics = [
      ...
      `LOCATION:${g.venue ? g.venue + ", " : ""}${g.city || ""}`,
      "END:VEVENT",
      "END:VCALENDAR"
    ].join("\r\n");
    const a = document.createElement("a");
    a.href = URL.createObjectURL(new Blob([ics], { type: "text/calendar" }));
    a.download = `UF_vs_${g.opponent.replace(/\s+/g, "_")}.ics`;
    a.click();
  }
  ...
  function renderRoster() {
    ...
    mount.innerHTML = rows
      ...
      <div class="card player" data-player="${esc(p.name)}">
        <div style="display:flex;gap:12px;align-items:center;">
          ...
          <div>
            <div><strong>${esc(p.name)}</strong> <span class="tiny muted">#${esc(p.number || "")} ${esc(p.pos || "")}</span></div>
            <div class="meta">${esc(p.hometown || "")}</div>
          </div>
    ...
    $$("#rosterGrid .player").forEach((card) =>
      card.addEventListener("click", () => {
        const p = STATE.stats.players.find((x) => x.name === card.dataset.player);
        openPlayerModal(p);
      })
    );
  }
  ...
  function renderTeamStats() {
    ...
    $("#teamStats").innerHTML = `
      <table><thead><tr>
        <th>PTS/G</th><th>REB/G</th><th>AST/G</th><th>STL/G</th><th>BLK/G</th><th>FG%</th><th>3P%</th><th>FT%</th>
      </tr></thead><tbody><tr>
        <td class="stat">${fmt1(t.pointsPerGame)}</td>
        <td>${fmt1(t.reboundsPerGame)}</td>
        <td>${fmt1(t.assistsPerGame)}</td>
        <td>${fmt1(t.stealsPerGame)}</td>
        <td>${fmt1(t.blocksPerGame)}</td>
        <td>${pct3(t.fieldGoalPct)}</td>
        <td>${pct3(t.threePointFieldGoalPct)}</td>
        <td>${pct3(t.freeThrowPct)}</td>
      </tr></tbody></table>`;
  }

  function renderPlayerStats() {
    const q = $("#playerFilter")?.value.toLowerCase() || "";
    let rows = STATE.stats.players.slice();
    if (q) rows = rows.filter((p) => p.name.toLowerCase().includes(q));
        ...
        <tbody>
          ${rows
            ...
            <tr class="plink" data-player="${esc(p.name)}">
              <td><div style="display:flex;align-items:center;gap:8px">${p.headshot ? `<img src="${p.headshot}" width="20" height="20" style="border-radius:50%">` : ""}<strong>${esc(p.name)}</strong></div></td>
              <td>${p.gp ?? "—"}</td><td>${fmt1(p.mpg)}</td><td class="stat">${fmt1(p.ppg)}</td>
    ...
    $$("#playersTable .plink").forEach((tr) =>
      tr.addEventListener("click", () => {
        const p = STATE.stats.players.find((x) => x.name === tr.dataset.player);
        openPlayerModal(p);
      })
    );
  }
  ...
  $("#exportPlayersCsv")?.addEventListener("click", () => {
    const rows = [
      ["Player", "GP", "MPG", "PPG", "RPG", "APG", "SPG", "BPG", "TOV", "FG%", "3P%", "FT%"]
    ];
    STATE.stats.players.forEach((p) =>
      rows.push([p.name, p.gp, p.mpg, p.ppg, p.rpg, p.apg, p.spg, p.bpg, p.tpg, p.fgPct, p.threePct, p.ftPct])
    );
    const a = document.createElement("a");
    a.href = URL.createObjectURL(new Blob([toCSV(rows)], { type: "text/csv" }));
    a.download = "gators_players.csv";
    a.click();
  });
  ...
  function fillCompare() {
    ...
    B.innerHTML = "";
    STATE.stats.players.forEach((p) => {
      A.add(new Option(p.name, p.name));
      B.add(new Option(p.name, p.name));
    });
    if (STATE.stats.players[0]) A.value = STATE.stats.players[0].name;
    if (STATE.stats.players[1]) B.value = STATE.stats.players[1].name;
  }
  ...
  function drawCompare() {
    ...
    svg.push(
      `</g><g font-size="14"><text x="${pad}" y="${pad - 8}"><tspan fill="#0021A5">●</tspan> ${esc(
        a.name
      )}</text><text x="${pad + 200}" y="${pad - 8}"><tspan fill="#FA4616">●</tspan> ${esc(b.name)}</text></g></svg>`
    );
  ...
  function renderAnalytics() {
    const rows = STATE.analytics.slice().reverse(); // latest at bottom
    $("#analyticsWrap").innerHTML = rows.length
      ? `<table><thead><tr><th>Date</th><th>Opponent</th><th>H/A</th><th>Score</th><th>ORtg</th><th>DRtg</th><th>Pace</th><th>eFG%</th><th>TO%</th><th>Box</th></tr></thead><tbody>
        ${rows
          .map(
            (r) =>
              `<tr><td>${esc(r.date)}</td><td>${esc(r.opp)}</td><td>${r.at}</td><td>${esc(r.score)}</td><td class="stat">${r.ortg}</td><td>${r.drtg}</td><td>${r.pace}</td><td>${r.efg}</td><td>${r.toPct}</td><td><a class="boxlink" target="_blank" rel="noopener" href="${r.box}">↗</a></td></tr>`
          )
          .join("")}
      </tbody></table>`
    ...
    const w = 800,
      h = 200,
      pad = 24,
      vals = rows.map((r) => r.ortg).filter((v) => typeof v === "number");
    if (vals.length < 2) {
      $("#analyticsTrend").innerHTML = "";
      return;
    }
    const n = vals.length,
      min = Math.min(...vals, 80),
      max = Math.max(...vals, 130);
  ...
  function defaultLines() {
    return (STATE.stats.players || []).map((p) => ({
      player: p.name,
  ...
  function bankrollUI() {
    ...
    if ($("#usernameInput")) $("#usernameInput").value = getLS(LS.username, "") || "";
  }
  function upsertLeaderboard(user, bankroll) {
    const board = getLS(LS.board, []);
    const i = board.findIndex((b) => b.user === user);
    if (i >= 0) board[i].bankroll = bankroll;
    else board.push({ user, bankroll, updated: Date.now() });
  ...
  function renderLeaderboard() {
    ...
    $("#leaderboard").innerHTML = `<table><thead><tr><th>#</th><th>User</th><th>Bankroll</th><th>Updated</th></tr></thead><tbody>
      ${board
        .map(
          (b, i) =>
            `<tr><td>${i + 1}</td><td>${esc(b.user)}</td><td>$${Number(b.bankroll).toLocaleString()}</td><td>${new Date(
              b.updated || Date.now()
  ...
  function fillTicketGames() {
    ...
    sel.innerHTML =
      ...
      STATE.schedule
        .map(
          (g) =>
            `<option value="${esc(`${g.date}|${g.time}|${g.opponent}`)}">${esc(`${g.date} – ${g.opponent} (${g.at})`)}</option>`
        )
        .join("");
  }
  function renderPropsTable() {
    ...
    $("#propsTable").innerHTML = `<table><thead><tr><th>Player</th><th>PTS</th><th>REB</th><th>AST</th><th>P+R+A</th><th>Pick</th><th>Add</th></tr></thead><tbody>
      ${lines
        .map(
          (l) =>
            `<tr><td><strong>${esc(l.player)}</strong></td><td>${l.pts}</td><td>${l.reb}</td><td>${l.ast}</td><td>${l.pra}</td>
          <td><select data-player="${esc(l.player)}" class="pick">
  ...
  function renderBetSlip() {
    const slip = getLS(LS.openBets, []);
    $("#betList").innerHTML = slip.length
      ? slip
          .map(
            (b, i) =>
              `<div class="card"><div><strong>${esc(b.player)}</strong> — ${b.market.replace(
                "_",
                " "
              )} <span class="badge">@${b.odds}</span></div><div class="meta">Slip item #${i + 1}</div></div>`
          )
          .join("")
  ...
  function placeBets() {
    ...
    hist.push({ placedAt: Date.now(), wager, gameId, bets: slip, status: "pending" });
  ...
  $("#exportHistory")?.addEventListener("click", () => {
    const h = getLS(LS.history, []);
    const a = document.createElement("a");
    a.href = URL.createObjectURL(new Blob([JSON.stringify(h, null, 2)], { type: "application/json" }));
  ...
  $("#importHistory")?.addEventListener("change", async (e) => {
    ...
    setLS(LS.history, JSON.parse(text));
    renderHistory();
    ...
    $("#historyWrap").innerHTML = `<table><thead><tr><th>Placed</th><th>Game</th><th>Wager</th><th>Status</th><th>Payout</th><th>Picks</th></tr></thead><tbody>
      ${hist
        ...
        .map((t) => {
          ...
          const gameTxt = g
            ? `${g.date} vs ${g.opponent} ${g.result ? `(${g.result})` : ""} ${g.box ? ` • <a class="boxlink" href="${g.box}" target="_blank" rel="noopener">box ↗</a>` : ""}`
            : "—";
          return `<tr><td>${new Date(t.placedAt).toLocaleString()}</td><td>${gameTxt}</td><td>$${Number(t.wager).toLocaleString()}</td><td>${esc(
            t.status || "pending"
          )}</td><td>${t.payout ? "$" + Number(t.payout).toLocaleString() : "—"}</td><td>${t.bets
            .map((b) => `${esc(b.player)} (${b.market.replace("_", " ")})`)
            .join("<br/>")}</td></tr>`;
        })
        .join("")}
    </tbody></table>`;
  ...
  function getFavs() {
    try {
      return JSON.parse(localStorage.getItem("ghub_favs")) || [];
    } catch {
      return [];
    }
  }
  function setFavs(v) {
    localStorage.setItem("ghub_favs", JSON.stringify(v));
  }
  ...
  function renderCountdown() {
    const el = $("#nextGame");
    if (!el || !STATE.schedule.length) return;
    const upcoming = STATE.schedule
      .map((g) => ({ ...g, t: Date.parse(`${g.date} ${g.time} ET`) }))
      .filter((g) => !isNaN(g.t) && g.t > Date.now())
      .sort((a, b) => a.t - b.t)[0];
    if (!upcoming) {
      el.textContent = "Next game: TBA";
      return;
    }
    (function tick() {
      const diff = upcoming.t - Date.now();
      if (diff <= 0) {
        el.textContent = `Gameday: vs ${upcoming.opponent}!`;
        return;
      }
  ...
  /* ---------- Photos & News (unchanged) ---------- */
  function loadPhotos() {
    const PHOTOS = HERO_IMAGES.map((src, i) => ({ src, alt: i === 0 ? "Exactech Arena" : "Gators" }));
    $("#photoGrid").innerHTML = PHOTOS.map(
      (p) => `<div class="card"><img loading="lazy" src="${p.src}" alt="${esc(p.alt)}" class="photo"/><div class="meta">${esc(p.alt)}</div></div>`
    ).join("");
  }
  function loadNews() {
    fetch("https://floridagators.com/rss.aspx?path=mbball")
      .then((r) => r.text())
      .then((xml) => {
        const items = [];
        const itemRe = /<item[\s\S]*?<\/item>/gi,
          titleRe = /<title>([\s\S]*?)<\/title>/i,
          linkRe = /<link>([\s\S]*?)<\/link>/i,
          dateRe = /<pubDate>([\s\S]*?)<\/pubDate>/i,
          descRe = /<description>([\s\S]*?)<\/description>/i;
        (xml.match(itemRe) || [])
          .slice(0, 12)
          .forEach((b) => {
        ...
        $("#newsList").innerHTML = items
          .map(
            (n) =>
              `<article class="news-card"><h4><a href="${n.link}" target="_blank" rel="noopener">${esc(n.title)}</a></h4><p>${esc(
                n.summary || ""
              )}</p><p class="meta">${new Date(n.date).toLocaleString()}</p></article>`
          )
          .join("");
      })
      .catch(() => {
        $("#newsList").innerHTML =
          `<article class="news-card"><h4>Welcome</h4><p class="muted">Live schedule/roster/stats + analytics.</p></article>`;
      });
  }
  ...
  function fillTicketGames() {
    ...
    sel.innerHTML =
      ...
      STATE.schedule
        .map(
          (g) => `<option value="${esc(`${g.date}|${g.time}|${g.opponent}`)}">${esc(`${g.date} – ${g.opponent} (${g.at})`)}</option>`
        )
        .join("");
  }
